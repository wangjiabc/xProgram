...
*/
	...
	//Testing Page Stuff
	$('#loadContentTestButton').on("click",function(){
		var target = $('#loadContentTestText')[0].value;
		loadContent(target);
	});


	$("#addProjectAdd").on("click",function(){
		addProject({
	...
	$("#testAddMiniTask").on("click",function(){
		tasks.add(
			$("#testTask")[0].value,
			$("#testTaskState")[0].value
		);
	});
	$("#testRemoveMiniTask").on("click",function(){
		tasks.remove(
			$("#testTaskRemoveId")[0].value
		);
	});
	$("#testChangeMiniTask").on("click",function(){
		tasks.changeTask(
			$("#testTaskChangeId")[0].value,
			$("#testChangeTask")[0].value
		);
	});
	$("#testUpdateMiniTask").on("click",function(){
		tasks.updateState(
